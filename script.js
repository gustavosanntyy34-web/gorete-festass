    ...
    /* =====================================================
       VALIDAÇÃO EXTRA — NÃO PERMITE HORÁRIO QUE JÁ PASSOU HOJE
       Vale para bolo, docinhos, cupcakes, bolos caseirinhos e kit festa.
    ===================================================== */

    const CAMPO_DATA_POR_CAMPO_HORARIO = {
        horaRetirada: "dataRetirada",
        horaRetiradaDocinhos: "dataRetiradaDocinhos",
        horaRetiradaCupcakes: "dataRetiradaCupcakes",
        horaRetiradaCaseirinho: "dataRetiradaCaseirinho",
        horaRetiradaKitFesta: "dataRetiradaKitFesta"
    };

    function obterCampoDataDoHorario(campoHorario) {
        if (!campoHorario) return null;

        const idData =
            CAMPO_DATA_POR_CAMPO_HORARIO[campoHorario.id];
    ...
    function horarioRetiradaJaPassouHoje(campoHorario, valorHorario) {
        const campoData =
            obterCampoDataDoHorario(campoHorario);
        ...
        const dataSelecionada =
            String(campoData.value || "").trim();

        const horarioSelecionado =
            String(valorHorario || "").slice(0, 5);

        if (
            dataSelecionada !== hojeISOSite() ||
            !/^\d{2}:\d{2}$/.test(horarioSelecionado)
        ) {
            return false;
        }
        ...
        return retirada.getTime() <= agora.getTime();
    }
    ...
    function validarHorarioRetiradaCampo(
        ...
        if (horarioRetiradaJaPassouHoje(campo, valor)) {
            if (limparSeInvalido) {
                campo.value = "";
            }
            ...
            if (mostrarAviso) {
                mostrarMensagemHorarioCampo(
                    campo,
                    "Esse horário já passou. Escolha um horário futuro para retirada hoje."
                );
            }
    ...
    function textoRetiradaResumo(data, horario) {
        const dataOk = /^\d{4}-\d{2}-\d{2}$/.test(String(data || ""));
        const horaOk = /^\d{2}:\d{2}$/.test(String(horario || "").slice(0, 5));

        if (dataOk && horaOk) {
            return `${formatarDataSite(data)} às ${String(horario).slice(0, 5)}`;
        }

        if (dataOk) {
            return `${formatarDataSite(data)} — falta o horário`;
        }

        if (horaOk) {
            return `${String(horario).slice(0, 5)} — falta a data`;
        }

        return "—";
    }

    function atualizarProgressoResumo(id, pendencias = []) {
        const elemento = document.getElementById(id);
        if (!elemento) return;

        const icone = elemento.querySelector(".resumo-progresso-icone");
        const texto = elemento.querySelector(".resumo-progresso-texto");

        const lista = (pendencias || []).filter(Boolean);

        elemento.classList.toggle("completo", lista.length === 0);
        elemento.classList.toggle("incompleto", lista.length > 0);

        if (icone) {
            icone.textContent = lista.length === 0 ? "✓" : "○";
        }

        if (texto) {
            texto.textContent =
                lista.length === 0
                    ? "Pedido pronto para adicionar ao carrinho."
                    : `Falta: ${lista.join(", ")}.`;
        }
    }

    function configurarResumoCamposRetirada() {
        [
            ["dataRetirada", "horaRetirada", atualizarResumoBolo],
            ["dataRetiradaDocinhos", "horaRetiradaDocinhos", atualizarResumoDocinhos],
            ["dataRetiradaCupcakes", "horaRetiradaCupcakes", atualizarResumoCupcakes],
            ["dataRetiradaCaseirinho", "horaRetiradaCaseirinho", atualizarResumoCaseirinho],
            ["dataRetiradaKitFesta", "horaRetiradaKitFesta", atualizarResumoKitFesta]
        ].forEach(([idData, idHora, atualizador]) => {
            [idData, idHora].forEach(id => {
                const campo = document.getElementById(id);
                if (!campo || campo.dataset.resumoRetiradaAtivo === "true") return;

                campo.dataset.resumoRetiradaAtivo = "true";

                campo.addEventListener("input", atualizador);
                campo.addEventListener("change", atualizador);
                campo.addEventListener("blur", atualizador);
            });
        });

        const corBolo = document.getElementById("corBolo");
        if (corBolo && corBolo.dataset.resumoCorAtivo !== "true") {
            corBolo.dataset.resumoCorAtivo = "true";
            corBolo.addEventListener("input", atualizarResumoBolo);
            corBolo.addEventListener("change", atualizarResumoBolo);
            corBolo.addEventListener("blur", atualizarResumoBolo);
        }
    }

    document.addEventListener("DOMContentLoaded", function() {
        configurarResumoCamposRetirada();
    });
    ...
    function atualizarResumoBolo() {
        ...
        const cor =
            document.getElementById("corBolo")
                ?.value
                ?.trim() || "";
        ...
        const resumoCorBolo =
            document.getElementById("resumoCorBolo");

        if (resumoCorBolo) {
            resumoCorBolo.textContent = cor || "—";
        }

        const resumoRetiradaBolo =
            document.getElementById("resumoRetiradaBolo");

        if (resumoRetiradaBolo) {
            resumoRetiradaBolo.textContent =
                textoRetiradaResumo(data, horario);
        }

        atualizarProgressoResumo(
            "progressoResumoBolo",
            [
                !tamanho ? "tamanho" : "",
                !massa ? "massa" : "",
                recheios.length === 0 ? "recheio" : "",
                !cor ? "cor" : "",
                !topo ? "topo" : "",
                !data ? "data" : "",
                !horario ? "horário" : ""
    ...
    async function finalizarPedidoWhatsApp() {
        ...
        try {
            ...
            /*
             * RESUMO PROFISSIONAL PARA O WHATSAPP
             * A data e o horário são obtidos diretamente dos itens do carrinho.
             * Se houver mais de uma retirada, todas são exibidas.
             */
            const retiradas = [
                ...new Map(
                    carrinho
                        .filter(item => item?.data || item?.horario)
                        .map(item => {
                            const dataItem =
            ...
            mensagem +=
                `Um novo pedido foi realizado pelo site.\n\n`;
            ...
            if (retiradas.length === 1) {
                const retirada = retiradas[0];

                if (retirada.data) {
                    mensagem +=
                        `*Data da retirada:* ${formatarDataSite(retirada.data)}\n`;
                }
            ...
            else if (retiradas.length > 1) {
                mensagem +=
                    `\n*Retiradas do pedido:*\n`;

                retiradas.forEach((retirada, indice) => {
                    const dataTexto =
                        retirada.data
                            ? formatarDataSite(retirada.data)
                            : "Data não informada";
                    ...
                    mensagem +=
                        `${indice + 1}. ${dataTexto} às ${horarioTexto}\n`;
                });
            }
            ...
            mensagem +=
                `\nNo link acima estão todos os detalhes da encomenda: produtos, quantidades, sabores, observações, retirada e imagens de inspiração.\n`;

            mensagem +=
                `\n*Confira os detalhes do pedido antes de confirmar com o cliente.*\n`;
    ...
    function atualizarResumoDocinhos() {
        ...
        const dataResumo =
            document.getElementById("dataRetiradaDocinhos")?.value || "";

        const horarioResumo =
            document.getElementById("horaRetiradaDocinhos")?.value || "";

        const retiradaResumo =
            document.getElementById("resumoRetiradaDocinho");

        if (retiradaResumo) {
            retiradaResumo.textContent =
                textoRetiradaResumo(dataResumo, horarioResumo);
        }

        atualizarProgressoResumo(
            "progressoResumoDocinho",
            [
                !categoria ? "categoria" : "",
                !quantidade ? "quantidade" : "",
                !sabores.length ? "sabores" : "",
                !dataResumo ? "data" : "",
                !horarioResumo ? "horário" : ""
            ]
        );
    ...
    function atualizarResumoCupcakes() {
        ...
        const retiradaResumo =
            document.getElementById("resumoRetiradaCupcake");
    ...
    function atualizarResumoCaseirinho() {
        ...
        const retiradaResumo =
            document.getElementById("resumoRetiradaCaseirinho");
    ...
    function atualizarResumoKitFesta() {
        ...
        const dataResumo =
            document.getElementById("dataRetiradaKitFesta")?.value || "";

        const horarioResumo =
            document.getElementById("horaRetiradaKitFesta")?.value || "";

        const retiradaResumo =
            document.getElementById("resumoRetiradaKitFesta");
        ...
        const pagamentoResumo =
            document.querySelector('input[name="pagamentoKitFesta"]:checked');

        const saboresBoloResumo =
            saboresSelecionadosKit("bolo");

        const saboresDocinhosResumo =
            saboresSelecionadosKit("docinho");

        const limiteCupcakesResumo =
            limiteSaboresKit("cupcake");

        const saboresCupcakesResumo =
            saboresSelecionadosKit("cupcake");

        atualizarProgressoResumo(
            "progressoResumoKitFesta",
            [
                !kitFestaSelecionado ? "kit" : "",
                kitFestaSelecionado && limiteSaboresKit("bolo") > 0 && !saboresBoloResumo.length ? "recheio do bolo" : "",
                kitFestaSelecionado && limiteSaboresKit("docinho") > 0 && !saboresDocinhosResumo.length ? "sabores dos docinhos" : "",
                kitFestaSelecionado && limiteCupcakesResumo > 0 && !saboresCupcakesResumo.length ? "sabores dos cupcakes" : "",
                !pagamentoResumo ? "condição" : "",
                !dataResumo ? "data" : "",
                !horarioResumo ? "horário" : ""
            ]
        );
    ...
    /* =====================================================
       OBSERVAÇÃO DO CHECKOUT — TEXTO DINÂMICO PELO PEDIDO
       Funciona no desktop e no mobile.
    ===================================================== */

    function configurarObservacaoCheckoutDinamica() {
        const campo = document.getElementById("observacaoPedido");

        if (!campo) {
            return;
        }

        const ajuda = document.getElementById("textoAjudaObservacao");
        ...
        let placeholder =
            "Ex.: informe aqui qualquer detalhe ou orientação importante sobre o seu pedido...";

        let textoAjuda =
            "Deseja acrescentar algum detalhe ao seu pedido?";

        if (tipos.length === 1) {
            const tipo = tipos[0];

            if (tipo === "bolo-personalizado") {
                placeholder =
                    'Ex.: escrever “Parabéns Maria”, preferência de decoração, cores ou algum detalhe importante...';

                textoAjuda =
                    "Deseja informar algum detalhe sobre o bolo?";
            }

            else if (tipo === "docinhos") {
                placeholder =
                    "Ex.: preferência de embalagem, organização dos sabores ou alguma orientação sobre os docinhos...";

                textoAjuda =
                    "Deseja informar algum detalhe sobre os docinhos?";
            }

            else if (tipo === "cupcakes") {
                placeholder =
                    "Ex.: cores desejadas, tema, decoração, embalagem ou alguma orientação sobre os cupcakes...";

                textoAjuda =
                    "Deseja informar algum detalhe sobre os cupcakes?";
            }
                ...
                placeholder =
                    "Ex.: alguma preferência, detalhe para a retirada ou orientação sobre o bolo caseirinho...";

                textoAjuda =
                    "Deseja informar algum detalhe sobre o bolo caseirinho?";
            }
                ...
                placeholder =
                    "Ex.: tema da festa, cores, nome do aniversariante ou alguma orientação especial sobre o Kit Festa...";

                textoAjuda =
                    "Deseja informar algum detalhe sobre o Kit Festa?";
            }

            else if (tipo === "produto") {
                placeholder =
                    "Ex.: preferência de embalagem, identificação ou alguma orientação sobre os produtos do pedido...";

                textoAjuda =
                    "Deseja acrescentar alguma orientação sobre o pedido?";
            }
        }
    ...
    document.addEventListener(
        "DOMContentLoaded",
        configurarObservacaoCheckoutDinamica
    );
